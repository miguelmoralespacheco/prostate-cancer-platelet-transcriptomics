###############################################################################
# Build BoneMarrow platelet-associated single-cell reference
...
tissue_label <- "BoneMarrow"
prefix <- "BM"
manifest_path <- file.path(project_dir, "Inputs", "BoneMarrow", "Manifests", "manifest_BM_HiSeq9.tsv")
output_dir <- file.path(project_dir, "Results_BoneMarrow")
...
parameter_contract <- list(
  ...
  bone_marrow_platelet_cluster = true_platelet_cluster,
...
p_umap <- Seurat::DimPlot(
  ...
  ggplot2::ggtitle("Bone marrow UMAP clustering") +
  ggplot2::xlab("UMAP 1") +
  ggplot2::ylab("UMAP 2") +
...
p_umap <- set_point_style(p_umap, size = 0.3, alpha = 0.45)
save_plot_pdf(p_umap, file.path(figures_dir, "BM_UMAP_GLOBAL_clusters.pdf"), width = 5.7 / 2.54, height = 4.5 / 2.54)
...
p_umap_donor <- Seurat::DimPlot(
  ...
  ggplot2::ggtitle("Bone marrow UMAP by donor") +
  ggplot2::xlab("UMAP 1") +
  ggplot2::ylab("UMAP 2") +
  ggplot2::labs(color = NULL) +
  theme_pub() +
...
p_umap_donor <- set_point_style(p_umap_donor, size = 0.30, alpha = 0.55)

save_plot_pdf(
  p_umap_donor,
  file.path(figures_dir, "BM_UMAP_GLOBAL_byDonor.pdf"),
  width = 5.7 / 2.54,
  height = 4.5 / 2.54
)
...
p_qc <- Seurat::VlnPlot(obj_global_platelet, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3) +
  ggplot2::ggtitle("BM platelet-friendly QC") + theme_pub()
save_plot_pdf(p_qc, file.path(figures_dir, "BM_QC_GLOBAL_platelet_filtered.pdf"), width = 10, height = 4)
...
p_dot_lineage <- ggplot2::ggplot(
  ...
  ggplot2::ggtitle("Lineage marker expression across bone marrow clusters") +
  ggplot2::xlab(NULL) +
...
save_plot_pdf(
  p_dot_lineage,
  file.path(figures_dir, "BM_DotPlot_GLOBAL_markers_HYBRID.pdf"),
  width = 10 / 2.54,
  height = 5.5 / 2.54
)
...
safe_write_csv(genes_detected_by_cluster, file.path(tables_dir, "BM_platelet_genes_detected_by_cluster.csv"), row.names = FALSE)
...
if (length(score_cols) >= 2) {
  ...
  save_plot_grid_pdf(
    p_feat4,
    file.path(figures_dir, "BM_marker_module_featureplots.pdf"),
    width = 10 / 2.54,
    height = 8 / 2.54,
    ncol = 2
  )
} else {
  cat("Skipping diagnostic FeaturePlot 4 scores: fewer than two diagnostic scores were generated.\n")
}
...
if (!is.na(platelet_marker_module_col) && platelet_marker_module_col %in% colnames(obj_score_plot@meta.data)) {
  ...
  save_plot_pdf(
    p_vln_platelet,
    file.path(figures_dir, "BM_SUPP_Violin_PlateletMarkerModule_byCluster.pdf"),
    width = 6 / 2.54,
    height = 4.5 / 2.54
  )
...
cells_cluster4 <- colnames(obj_global_platelet)[
...
p_umap_true <- Seurat::DimPlot(
  obj_global_platelet,
  reduction = "umap",
  cells.highlight = cells_cluster4,
  ...
  ggplot2::ggtitle("Bone marrow low-RNA UMAP") +
  ggplot2::xlab("UMAP 1") +
  ggplot2::ylab("UMAP 2") +
  ggplot2::annotate(
    "text",
...
p_umap_true <- set_point_style(p_umap_true, size = 0.3, alpha = 0.45)

save_plot_pdf(
  p_umap_true,
  file.path(figures_dir, "BM_platelet_associated_lowRNA_cluster_UMAP.pdf"),
  width = 5.1 / 2.54,
  height = 4.5 / 2.54
)
...
safe_write_csv(gene_universe_raw, file.path(tables_dir, "BM_platelet_associated_lowRNA_cluster_gene_universe_raw.csv"), row.names = FALSE)
safe_write_csv(gene_universe_curated, file.path(tables_dir, "BM_platelet_associated_lowRNA_cluster_gene_universe_curated.csv"), row.names = FALSE)
safe_write_lines(gene_universe_raw$gene, file.path(tables_dir, "BM_platelet_associated_lowRNA_cluster_genes_raw.txt"))
safe_write_lines(gene_universe_curated$gene, file.path(tables_dir, "BM_platelet_associated_lowRNA_cluster_genes_curated.txt"))
...
obj_global_all <- Seurat::AddModuleScore(
  ...
  assay = "RNA",
  name = "platelet_associated_transcriptional_score_BM_universe"
)
score_col <- "platelet_associated_transcriptional_score_BM_universe1"
if (!(score_col %in% colnames(obj_global_all@meta.data))) stop("Score column not generated: ", score_col)
safe_write_csv(obj_global_all@meta.data, file.path(tables_dir, "BM_global_all_metadata_with_platelet_associated_transcriptional_score.csv"), row.names = TRUE)
safe_save_rds(obj_global_platelet, file.path(objects_dir, "BM_obj_global_platelet.rds"))
safe_save_rds(obj_global_all, file.path(objects_dir, "BM_obj_global_all_with_platelet_associated_transcriptional_score.rds"))
...
p_vln_score_donor <- Seurat::VlnPlot(
  ...
  ggplot2::ggtitle("BM universe platelet score by donor") +
  ggplot2::xlab(NULL) +
...
save_plot_pdf(p_vln_score_donor, file.path(figures_dir, "BM_platelet_associated_transcriptional_score_by_donor.pdf"), width = 5.2, height = 3.5)
...
safe_write_csv(score_summary, file.path(tables_dir, "BM_platelet_associated_transcriptional_score_summary.csv"), row.names = FALSE)

cat("Completed BoneMarrow reference build.\n")
